...
class WindowFacadeTests: XCTestCase {
    ...
    func test_didRequestSearch_sendsCorrectMessages() {
        let (sut, webView, presenter, delegate) = makeSUT()
    ...
    func test_didLoadPage_sendsCorrectMessages() {
        ...
        XCTAssertEqual(delegate.receivedMessages, [.saveToHistory])
        XCTAssertEqual(webView.receivedMessages, [])
    }
    ...
    func test_updateSafelist_withURLEnabled_addURLToSafelist() {
        ...
        XCTAssertEqual(delegate.receivedMessages, [.saveDomainToSafeList(url)])
        XCTAssertEqual(webView.receivedMessages, [])
        XCTAssertEqual(presenter.receivedMessages, [])
    }
    ...
    func test_updateSafelist_withURLDisabled_removesURLFromSafelist() {
        ...
        XCTAssertEqual(delegate.receivedMessages, [.removeDomainFromSafeList(url)])
        XCTAssertEqual(webView.receivedMessages, [])
        XCTAssertEqual(presenter.receivedMessages, [])
    }
    ...
    private func makeSUT() -> (sut: WindowFacade, webView: WebViewSpy, presenter: WindowPresenterSpy, delegate: WindowFacadeSpy) {
        ...
        return (sut, webView, presenter, delegate)
    }
}

private class WindowFacadeSpy: WindowFacadeDelegate {
    enum Message: Equatable {
        case saveDomainToSafeList(_ domain: String)
        case removeDomainFromSafeList(_ domain: String)
        case saveToHistory
    }
    ...
    func saveDomainToSafeList(_ domain: String) {
        receivedMessages.append(.saveDomainToSafeList(domain))
    }
    
    func removeDomainFromSafeList(_ domain: String) {
        receivedMessages.append(.removeDomainFromSafeList(domain))
    }
    
    func saveToHistory(_ page: WebPage) {
        receivedMessages.append(.saveToHistory)
    }
}
